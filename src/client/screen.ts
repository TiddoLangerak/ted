...
export function startAlternateBuffer() {
  ...
  ttyOut.write(ansiEscapes.cursorHide);
}

export function closeAlternateBuffer() {
  ttyOut.write("\x1b[?1049l");
  ttyOut.write(ansiEscapes.cursorShow);
}
...
export function draw(immediate: boolean = false, forceRedraw: boolean = false) {
  ...
  const buffer: Buffer = Array.from(new Array(ttyOut.getRows()), () =>
    Array.from(new Array(ttyOut.getColumns()), () => ({
      ch: " ",
      modifiers: new Set()
    }))
  );
  drawLayers(screenLayout, buffer);
