import { StdWritable } from "./stdio";
import ansiEscapes from "ansi-escapes";
import screenLayout, { Layer } from "./screenLayout";
...
export class Screen {
  ...
  private ttyOut : StdWritable;
  private drawables = new Map();
  constructor(ttyOut: StdWritable) {
    this.ttyOut = ttyOut;
  }
  private drawBuffer(buffer: Buffer, forceRedraw = false) {
    const rows: string[] = [];
    buffer.forEach((row, idx) => {
      const rowTokens = [ansiEscapes.cursorTo(0, idx), ansiEscapes.eraseLine];
      ...
      const closeMods = Array.from(modifiers)
        .map(mod => mod.close)
        .join("");
    ...
    const tokenString = rows
      .filter(
        ...
        // been requested
        forceRedraw || this.currentRows.length < idx || this.currentRows[idx] !== row
      )
      .join("\n");
    this.ttyOut.write(tokenString);
  ...
  closeAlternateBuffer() {
    this.ttyOut.write("\x1b[?1049l");
    this.ttyOut.write(ansiEscapes.cursorShow);
  }
   ...
   */
  draw(immediate: boolean = false, forceRedraw: boolean = false) {
    ...
    if (!immediate) {
      process.nextTick(() => this.draw(true, forceRedraw));
      return;
    }
    ...
    const buffer: Buffer = Array.from(new Array(this.ttyOut.getRows()), () =>
      Array.from(new Array(this.ttyOut.getColumns()), () => ({
        ch: " ",
        modifiers: new Set()
      }))
    );
    this.drawLayers(screenLayout, buffer);
    this.drawBuffer(buffer, forceRedraw);
  }

  initialize() {
    this.startAlternateBuffer();

    process.on("exit", () => {
      this.closeAlternateBuffer();
    });
    ...
    process.on("SIGWINCH", () => this.draw(false, true));
    this.isInitialized = true;
  }
  drawLayers(layers: Layer[], buffer: Buffer) {
    layers.forEach(layer => {
      if (typeof layer === "string") {
        const drawable = this.drawables.get(layer);
        if (!drawable) {
        ...
        const layerSize = getLayerSize(layer, buffer);

        // Number of buffers that have an auto height
        const autoCount = layer.buffers.filter(buff => buff.size === "auto")
          .length;
        ...
        layer.buffers.reduce((screenBuffer, buff) => {
          ...
          if (layer.split === "vertical") {
            ...
            this.drawLayers(buff.layers, virtualBuffer);
          ...
          this.drawLayers(buff.layers, left);
          return right;
        }, buffer);
      }
    });
  ...
  registerDrawable(
    name: string,
    drawFunction: (buff: Buffer) => unknown
  ) {
    this.drawables.set(name, drawFunction);
  }
}
